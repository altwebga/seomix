...
function Particles({ color, count = 100 }: { color: string; count?: number }) {
  ...
  const particleElapsedRef = React.useRef(0)

  useFrame((_, delta) => {
    particleElapsedRef.current += delta
    const elapsed = particleElapsedRef.current
      ...
      if (Math.floor(elapsed * 60) % 2 === 0) {
        ...
        if (positionAttr) {
          ...
          for (let i = 0; i < count; i++) {
            ...
            positions[i * 3 + 1] = initialYPositions.current[i] + Math.sin(elapsed + i * 0.1) * 0.5
          }
...
function LightBeams({ color }: { color: string }) {
  ...
  const beamElapsedRef = React.useRef(0)

  useFrame((_, delta) => {
    beamElapsedRef.current += delta
    if (materialRef.current) {
      // Animate opacity for all beams at once
      materialRef.current.opacity = 0.3 + Math.sin(beamElapsedRef.current * 3) * 0.15
    }
  })
...
function CameraController() {
  ...
  const camElapsedRef = React.useRef(0)

  useFrame((_, delta) => {
    camElapsedRef.current += delta
    const time = camElapsedRef.current * 0.1
