...
import placeholderImage from '@/public/images/placeholder_image.svg'
  ...
  export async function getPost(slug: string): Promise<Post | null> {
    try {
        ...
        const post = posts[0]; // Берем первый пост из массива

        if (post.featured_media) {
            try {
                // Запрос для получения URL изображения по featured_media
                const imageResponse = await fetch(`${apiUrl}/media/${post.featured_media}?_fields=source_url`, { next: { revalidate: 3600 } });
                const imageData = await imageResponse.json();
